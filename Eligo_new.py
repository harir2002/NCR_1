    ...
    try:
        with st.spinner(f"Generating {report_type} NCR Report..."):
            ...
            def extract_modules_from_description(description):
                ...
                range_patterns = [
                    r"module[-\s]*(\d+)[-\s]*(?:to|-)[-\s]*(\d+)",
                ]
                for pattern in range_patterns:
                    range_matches = re.findall(pattern, description_lower, re.IGNORECASE)
                    for start_str, end_str in range_matches:
                        try:
                            start_num = int(start_str)
                            end_num = int(end_str)
                            if start_num <= end_num and end_num - start_num <= 10:
                                modules.update(f"Module {i}" for i in range(start_num, end_num + 1))
                        except ValueError:
                            continue

                # Pattern 2: Handle combinations like "Module 1 & 2" or "Module – 6 & 7"
                combination_patterns = [
                    r"module[-\s]*(?:–|-)?\s*(\d+)\s*[&and]+\s*(\d+)",
                    r"module[-\s]*(?:–|-)?\s*(\d+)\s*[,&]\s*(\d+)",
                ]
                ...
                # Pattern 3: Handle comma/and separated lists like "Module 1, 2, 3"
                list_pattern = r"module[-\s]*(?:–|-)?\s*([0-9,\s&and]+)"
                list_matches = re.findall(list_pattern, description_lower, re.IGNORECASE)
                ...
                # Pattern 4: Handle individual module references - FIXED PATTERNS
                individual_patterns = [
                    r"module\s*[-–]?\s*(\d+)(?![0-9])",  # Matches "Module - 6", "Module – 6", "Module 6"
                    r"module\s+(\d+)(?![0-9])",          # Matches "Module 6" with space
                    r"module[-–]\s*(\d+)(?![0-9])",      # Matches "Module-6" or "Module–6"
                ]
                for pattern in individual_patterns:
                    individual_matches = re.findall(pattern, description_lower, re.IGNORECASE)
            ...
            def determine_tower_assignment(description):
                ...
                tower_matches = re.findall(r"tower\s*[-\s]*([fgh])", description_lower, re.IGNORECASE)
                ...
                multiple_tower_pattern = re.search(
                    r"tower\s*[-\s]*([fgh])\s*[,&]?\s*(?:and\s+)?(?:tower\s*[-\s]*)?([fgh])",
