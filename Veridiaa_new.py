        ...
        with st.spinner(f"Generating {report_type} NCR Report..."):
            ...
            for record in processed_data:
                try:
                    ...
                    # FIXED MODULE EXTRACTION LOGIC - This is the only change
                    if common_pattern.search(description):
                        ...
                        range_pattern = r"(?:module|mod|m)[-\s]*(\d+)[-\s]*(?:to|-)[-\s]*(\d+)"
                        ...
                        for start_str, end_str in range_matches:
                            try:
                                ...
                                if start_num <= end_num and end_num - start_num <= 20:  # Reasonable limit
                                    modules.update(f"M{i}" for i in range(start_num, end_num + 1))
                            except ValueError:
                                continue
                        
                        # Pattern 2: Handle comma/and separated lists like "1,2, 3 & 4", "1, 2 and 3"
                        if not modules:
                            list_pattern = r"(?:module|mod)[-\s]*([0-9,\s&and]+)"
                            ...
                            for match in list_matches:
                                ...
                                for num_str in numbers:
                                    try:
                                        num = int(num_str)
                                        if num <= 50:  # Reasonable module number limit
                                            modules.add(f"M{num}")
                                    except ValueError:
                                        continue
                        
                        # Pattern 3: Handle individual module references like "M1", "mod 5"
                        if not modules:
                            # More specific pattern to avoid matching numbers that are part of other contexts
                            individual_pattern = r"(?:module|mod|m)[-\s]*(\d+)(?!\s*(?:to|-)\s*\d+)"
