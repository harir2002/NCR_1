...
def assign_site(description: str, standard_sites: list) -> list:
    """
    Assigns a site based on description, handling block patterns (e.g., Block 1, B1), facility names, and returning Common Area if none match.
    Args:
    ...
    # Normalize description: lowercase, replace hyphens/commas/ampersands with spaces
    desc = re.sub(r'[-,&]', ' ', description.lower()).strip()

    # Initialize matched sites
    matched = set()

    # Shorthand patterns for blocks and facilities
    shorthand_patterns = {
    ...
    # Check for full site names or close matches
    if not matched:
        for site in standard_sites:
            ...
            if re.search(rf'\b{site_pattern}\b', desc):
                ...
                # Check for key facility words (e.g., "banquet" for Banquet Hall)
                facility_words = site.lower().split('(')[1].split(')')[1].strip().split()
